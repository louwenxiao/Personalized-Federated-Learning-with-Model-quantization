...
def main():
    ...
    model2 = models.create_model_instance(common_config.dataset_type)
    model2.to(device)
    ...
    deta = torch.zeros_like(local_para)
    ...
    for epoch in range(1, 1+common_config.epoch):
        ...
        # 获得全局模型
        # time.sleep(100)
        para_delta,model = get_data_socket(master_socket)
        deta = para_delta.clone().detach().to(device)
        torch.nn.utils.vector_to_parameters(model.to(device), local_model.parameters())     
        # 计算本地模型的梯度与全局模型梯度之间的差
        accumulated_error = accumulated_error + (1.0/local_iters)*(model_update-deta)   # 更新本地误差
        print("量化后的误差为：",torch.norm(accumulated_error))
        time5 = time.time()
...
def model_quantization(local_para, ratio):
    if ratio == None:
        ...
        return Quantize_model(local_para,bits=2**(ratio-1))
    
    return local_para

def Quantize_model(grad_para,bits=128,bucket_size=4096):
    grad_para = grad_para.to("cpu")
    len_grad = len(grad_para)
    data00 = grad_para.detach().numpy()
    num_bucket = int(len_grad/ bucket_size)
    if num_bucket*bucket_size+bucket_size-len_grad>0:
        num_bucket = num_bucket +1
        arr2 = [0]*(num_bucket*bucket_size-len_grad)
        data00 = np.concatenate((data00, arr2))
    data = torch.tensor(data00)   # 数据变成可以整除的数字
    data2 = data.view(-1,bucket_size)
    random_grad = torch.rand(len(data))-torch.ones(len(data))*0.5
    random_grad = random_grad.view(-1,bucket_size)        # 随机数

    norm = torch.norm(data2,dim=1).reshape((num_bucket,1))
    data5 = torch.round(data2/norm * bits+random_grad)
    data7 = norm*data5/bits
    data8 = data7.view(-1)
    data0 = list(data8.detach().numpy())
    return torch.tensor(data0[0:len_grad]).to(device)
