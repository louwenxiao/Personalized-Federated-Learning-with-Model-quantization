...
def main():
    ...
    for epoch_num in range(1, 1+common_config.epoch):
        ...
        print("epoch-{} lr: {}".format(epoch_num, epoch_lr))
        ...
        # 发送全局模型到每一个worker
        para_delta,init_para = aggregate_model(init_para,worker_list,epoch_lr,aggregate_vertor)
        communication_parallel(worker_list, action="send_model",data=(para_delta,init_para))
        ...
        pd.DataFrame(RESULT).to_csv('/data/wxlou/PS_mywork/FedCOM/result/'+
                            '/{}_FedCOM_{}_{}_lr{}_decay_rate{}_batch{}.csv'.format(start_time,
                            args.data_pattern,args.dataset_type,args.lr,args.decay_rate,args.batch_size))    
...
def partition_data(dataset_type, data_pattern, worker_num=20):      # 使用6个用户训练，需要添加一个IID
    ...
    partition_sizes = non_iid_partition(0.8, worker_num=worker_num)
...
def aggregate_model(local_para, worker_list,epoch_lr,aggregate_vertor):
    with torch.no_grad():
        para_delta = torch.zeros_like(local_para)
        average_weight=1.0/len(worker_list)
        for worker in worker_list:
            mid_model = worker.model_update
            para_delta += mid_model * average_weight
        new_para = local_para + para_delta

    return para_delta,new_para
