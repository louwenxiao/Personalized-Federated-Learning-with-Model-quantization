...
def main():
    ...
    for epoch in range(1, 1+common_config.epoch):
        ...
        #     train_loss = train(local_model, train_loader, optimizer, local_iters=local_iters,device=device)
        # else:
        #     train_loss = train(local_model, train_loader, optimizer, local_iters=local_iters,device=device,model_type=model2)
        ...
        train_and_send_time = model_size * 4 / computation
        ...
        # 发送模型更新、训练时间和流量
        # send_data_socket((new_para,grad_para,train_time,traffic), master_socket)
        send_data_socket((grad_para,train_time,traffic), master_socket)
